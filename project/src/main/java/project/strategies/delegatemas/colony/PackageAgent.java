...
public class PackageAgent implements TickListener, SimulatorUser, CommunicationUser {
	...
	if (lastFeasibilityCheck > Settings.TIMESTEPS_WAIT_BEFORE_SENDING_FEASIBILITY_ANTS) {
	    System.out.println(getId()+": Sending Feasibility Ants");
	    sendFeasibilityAnts();
	...
	if (message instanceof FeasibilityAnt) {
		...
		if (fAnt.getHopsLeft()-1 > 0) {
		    communicationAPI.broadcast(
			    new FeasibilityAnt(this, new Path(fAnt.getPath(), this), fAnt.getHopsLeft() - 1),
	...
	for (int i=0; i <pathTable.getPaths().size(); i++) {
	    string += "\n"+pathTable.getPaths().get(i).toString();
	}
